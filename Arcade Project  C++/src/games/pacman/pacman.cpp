...
void Pacman::randomMove(int content)
{
    ...
    for (int y = 0; y < this->_height; y++) {
        for (int x = 0; x < this->_width; x++) {
            ...
            if (this->_map[y + 1][x + 1] == _contentChar[content])
                position.set(x, y);
        }
        // std::cout << std::endl;
    }
    ...
    this->_map[pos.getY() + 1][pos.getX() + 1] = _contentChar[content - GHOST1];
    object::Sprite_t sprite;
    for (std::size_t i = 0; i < this->_objects.size(); i++)
        if (this->_objects[i].getContent() == content)
            sprite = this->_objects[i].getSprite();
    if (this->_eatingGhost > 0)
                        ...
                        if (this->_eatingGhost > 0) {
                            for (std::size_t j = 0; j < this->_objects.size(); j++)
                                if (this->_objects[j].getContent() == content) {
                                    this->_objects[j].setPos(Vector2i(11,13));
                                    this->_map[13 + 1][11 + 1] = _contentChar[content];
                                }
                            this->_eatingGhost = 0;
                            this->_previousMovePacMan = ' ';
                        } else {
...
bool Pacman::update(Vector2i mouse, char key)
{
    ...
    if (this->_clock == 24) {
        ...
        restart = this->IAGhost(GHOST1);
        restart = this->IAGhost(GHOST2);
        restart = this->IAGhost(GHOST3);
        restart = this->IAGhost(GHOST4);
        if (restart) {
            this->_restart = true;
            return false;
        }
